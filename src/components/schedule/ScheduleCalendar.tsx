...
export const ScheduleCalendar = ({
  ...
  if (view === 'day') {
    return <div className="fixlyfy-card overflow-hidden">
        ...
        <div className="overflow-auto">
          ...
          return <div key={time} className={cn("grid grid-cols-[100px_1fr] border-b border-fixlyfy-border min-h-[100px]", index % 2 === 0 ? "bg-white" : "bg-fixlyfy-bg-interface/30")}>
                <div className="p-3 border-r border-fixlyfy-border flex items-start">
                  <span className="text-fixlyfy-text-secondary">{time}</span>
                </div>
                <div className="p-2">
                  {jobsInSlot.map(job => <div key={job.id} onClick={() => handleJobClick(job.id)} className={cn("p-2 rounded mb-2 text-white shadow-sm cursor-pointer hover:opacity-90 transition-opacity", job.status === 'scheduled' && "bg-fixlyfy", job.status === 'in-progress' && "bg-fixlyfy-warning", job.status === 'completed' && "bg-fixlyfy-success")}>
                      <div className="flex justify-between items-start">
                        ...
                        <Badge className={cn("bg-white/20", job.status === 'scheduled' && "text-white", job.status === 'in-progress' && "text-white", job.status === 'completed' && "text-white")}>
                          {job.technician.name}
                        </Badge>
                      </div>
                    </div>)}
    ...
    return <div className="fixlyfy-card overflow-hidden">
        ...
        <div className="overflow-auto">
          {timeSlots.map((time, timeIndex) => <div key={time} className={cn("grid grid-cols-7 border-b border-fixlyfy-border min-h-[100px]", timeIndex % 2 === 0 ? "bg-white" : "bg-fixlyfy-bg-interface/30")}>
            ...
            return <div key={day.toString()} className={cn("p-2 border-r border-fixlyfy-border last:border-r-0 relative", dayIndex === 0 && "pl-[100px]")}>
                    {dayIndex === 0 && <div className="absolute left-0 top-3 w-[90px] text-center text-fixlyfy-text-secondary">
                        {time}
                      </div>}
                    {jobsInSlot.map(job => <div key={job.id} onClick={() => handleJobClick(job.id)} className={cn("p-2 rounded mb-2 text-white shadow-sm text-xs cursor-pointer hover:opacity-90 transition-opacity", job.status === 'scheduled' && "bg-fixlyfy", job.status === 'in-progress' && "bg-fixlyfy-warning", job.status === 'completed' && "bg-fixlyfy-success")}>
                        <div className="font-medium">{job.title} - {job.client}</div>
  ...
  return <div className="fixlyfy-card overflow-hidden">
      ...
      <div className="grid grid-cols-7 gap-0">
        ...
        return <div key={i} className={cn("h-24 p-1 border-r border-b border-fixlyfy-border relative overflow-hidden", jobsForDay.length > 0 ? "bg-fixlyfy-bg-interface/30" : "")}>
              <div className="text-xs font-medium mb-1">{dayNum}</div>
              {jobsForDay.slice(0, 3).map(job => <div key={job.id} onClick={() => handleJobClick(job.id)} className={cn("text-xs p-1 rounded text-white mb-1 cursor-pointer hover:opacity-90 transition-opacity whitespace-nowrap overflow-hidden text-ellipsis", job.status === 'scheduled' && "bg-fixlyfy", job.status === 'in-progress' && "bg-fixlyfy-warning", job.status === 'completed' && "bg-fixlyfy-success")}>
                  {job.title} - {format(job.date, 'h:mm a')}
                </div>)}
              {hasExcessJobs && <div className="text-xs text-fixlyfy cursor-pointer hover:underline absolute bottom-1 right-1 bg-fixlyfy-bg-interface/80 px-2 py-1 rounded" onClick={() => {
