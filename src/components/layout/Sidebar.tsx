...
export function Sidebar({
  ...
  return <div className={cn("pb-12 w-64 bg-white border-r border-fixlyfy-border shadow-sm", className)}>
      <div className="space-y-4 py-4">
        <div className="px-3 py-2">
          <div className="mb-6 px-4 py-2">
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold bg-gradient-primary bg-clip-text text-transparent tracking-tight transform hover:scale-105 transition-transform duration-300" style={{
          ...
          <ScrollArea className="h-[calc(100vh-8rem)]">
            <div className="space-y-1">
              {routes.map(route => <TrackingWrapper key={route.href} actionType="navigation" element="sidebar_menu" context={{
              destination: route.href,
              label: route.label
            }}>
                  <Button variant={location.pathname === route.href ? "default" : "ghost"} className={cn("w-full justify-start text-sm font-medium transition-all duration-200 hover:scale-[1.02]", location.pathname === route.href ? "bg-gradient-primary text-white shadow-md hover:shadow-lg" : "text-fixlyfy-text hover:bg-fixlyfy/10 hover:text-fixlyfy")} onClick={() => handleNavigation(route.href)}>
