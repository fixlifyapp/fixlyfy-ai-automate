...
serve(async (req) => {
  ...
  try {
    ...
    // Add the phone number +1 833-574-3145 as purchased
    ...
    if (!existingNumber) {
      ...
      const { error: insertError } = await supabaseClient
        ...
        .insert({
          ...
          connect_instance_id: Deno.env.get('AMAZON_CONNECT_INSTANCE_ID'),
          ai_settings: {
      ...
      // Update existing number to ensure AI dispatcher is enabled
    ...
    // Ensure AI agent config exists for the user
    ...
    if (!existingConfig) {
      ...
      const { error: configInsertError } = await supabaseClient
        ...
        .insert({
          ...
          custom_prompt_additions: 'You are helping customers with field service requests including HVAC, plumbing, electrical, and general maintenance services.',
          connect_instance_arn: `arn:aws:connect:${Deno.env.get('AWS_REGION')}:${Deno.env.get('AMAZON_CONNECT_INSTANCE_ID')}`,
    ...
    if (!existingCreds) {
      // Create AWS credentials record (using environment variables)
    ...
    return new Response(JSON.stringify({
      ...
      phone_number: phoneNumber,
      ai_enabled: true,
      connect_instance: Deno.env.get('AMAZON_CONNECT_INSTANCE_ID')
    }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' }
    })
